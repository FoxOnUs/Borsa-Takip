...
const StockListPage: React.FC = () => {
  ...
  const fetchSymbolsAndData = async () => {
    ...
    const lastCacheTime = localStorage.getItem(
      `${STOCK_DATA_CACHE_KEY}_timestamp`
    );

    if (
      cachedData &&
      lastCacheTime &&
      Date.now() - Number(lastCacheTime) < CACHE_EXPIRY_TIME_MS
    ) {
      try {
    ...
    try {
      ...
      if (!symbolsResponse.ok) {
        throw new Error(
          `Failed to fetch stock symbols: ${symbolsResponse.status} ${symbolsResponse.statusText}`
        );
      }
      ...
      for (const symbol of symbolsData) {
        try {
          ...
          if (intradayData.symbol !== symbol) {
            console.error(
              `Symbol mismatch: expected ${symbol}, got ${intradayData.symbol}`
            );
          ...
          if (historyData.symbol !== symbol) {
            console.error(
              `Symbol mismatch: expected ${symbol}, got ${historyData.symbol}`
            );
            historyData.symbol = symbol;
          }

          const processedItem = processStockData(
            symbol,
            intradayData,
            historyData
          );
      ...
      localStorage.setItem(
        STOCK_DATA_CACHE_KEY,
        JSON.stringify(stockListItemsUpdated)
      );
      localStorage.setItem(
        `${STOCK_DATA_CACHE_KEY}_timestamp`,
        Date.now().toString()
  ...
  const processStockData = (
    ...
    if (intradayData.error) {
      ...
      try {
        const timeSeriesKey = Object.keys(intradayData).find(
        ...
        if (timeSeriesKey && historyData[timeSeriesKey]) {
          ...
          if (timePoints.length > 0) {
            const previousDataKey =
              timePoints.length > 1 ? timePoints[1] : timePoints[0];
    ...
    if (
      intradayPrice !== undefined &&
      lastDayClose !== undefined &&
      intradayPrice !== null &&
      lastDayClose !== null
    ) {
            ...
            <li key={item.symbol} className="py-4">
              <NextLink legacyBehavior href={`/stockdetails/${item.symbol}`}>
                <a className="block">
                    ...
                    {item.error ? (
                      ...
                      <div>
                        {item.intradayPrice !== undefined &&
                          item.intradayPrice !== null && (
                            <p className="text-gray-500">
                              ${item.intradayPrice.toFixed(2)}
                            </p>
                          )}
                        {item.percentageChange !== undefined &&
                          item.percentageChange !== null && (
                            <p
                              className={`text-sm font-medium ${item.percentageChange >= 0 ? "text-green-500" : "text-red-500"}`}
                            >
                  ...
                  {item.lastDayClose !== undefined &&
                    item.lastDayClose !== null &&
                    item.intradayPrice !== undefined &&
                    item.intradayPrice !== null &&
                    !item.error && (
